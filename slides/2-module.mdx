...
![browseriify](https://browserify.org/images/browserify.png)
...
- Node.js のコードをブラウザで実行可能なものに変換する Bundler の概念が登場
- Browserify が有名
...
- 問題:
  - bundle された file は単一な巨大なファイル
    - 数 10 MB、数 100 MB
    - ユーザーが確認したいページとは関係のないコードも含まれている
      - 閲覧したいのはログイン画面だが、マイページ・トップなど、あらゆる画面向けの JS を読みこんでしまう
    - これをサイトを閲覧しているユーザーに毎アクセス読み込ませる...!? :thinking_face:
  - 少しコードを修正しただけでも、プロジェクト内の全ての JS を bundle し直す必要があった
    - 1 行 の typo を直す => bundle の完了を 10 分待つ
...
```js
// AMD
define(‘myModule’, [‘lodash’, ‘someDep’],
function (_, someDep) {
   	return { … }
	}
);
...
const foo = "foo";
// module のdefault export
export default foo;
```

```js
// module のimport
import {bar} from ‘someModule’
...
create-react-app の webpack.config.js
...
- 複雑な Webpack の config を書かなくとも、zero config で実行環境を作れるようにする動き
  - React.js だと Next.js など
- Internet Explorer をサポートする必要がなくなったので、古いブラウザをサポートするためだけに Webpack を採用しなくても良くなった
- build の時間を短縮したい
  - :thinking_face: bundler は JavaScript で実装されていなくても良いのでは？
  - rust / go 製の build tool の登場
  - 例: esbuild / swc
  - Next.js を採用するか、内部的に esbuild を利用している Vite で環境を作るのか、の 2 択になることが多い
